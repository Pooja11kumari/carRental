...
const Sidebar = () => {
    ...
    const updateImage=async()=>{
        try{
            ...
            const {data}=await axios.post('/upload/owner/update-image',formData)
            if(data.success){
                fetchUser();
    ...
    <div className='relative min-h-screen md:flex flex-col items-center pt-8 max-w-13 md:max-w-60 w-full border-borderColor text-sm'>
        <div className='group relative'>
            <label htmlFor="image">
                ...
                } alt="" className='h-28 w-28 md:h-32 md:w-32 rounded-full mx-auto object-cover border-2 border-primary'/>
